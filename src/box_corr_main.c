 ...
 *
 * MULTIPLE box widths: --h takes a comma list (e.g. 0.1,0.25,0.5,1,2).  The
 * box width only changes how a configuration is averaged, NOT the (expensive)
 * trajectory, so EACH realization is measured at every h from the SAME
 * trajectory — all h for ~the price of one.  One box_corr.txt per h is written
 * to <out>/h<value>/.  The trajectory RNG does not depend on h, so each h's
 * output is bit-identical to an independent single-h run at that h.
 *
 ...
 * gamma = (N^2-nu^2)/(N^2+nu^2).  Macroscopic time tau = micro/N^2.  Validate:
 *   python3 python_scripts/mc_box_correlation.py --plot-only <out>/h<value>
 *   python3 python_scripts/validate_iface.py <out>/h<value>
 *
 * Usage:
 *   ./BoxCorr [--N 12] [--L 4] [--nu 1.0] [--h 0.5] [--M 15]
 *             [--T 0.05,0.1,0.2,0.4] [--nsims 1000] [--seed 0]
 *             [--observable spin|iface] [--checkpoint-interval 5] [--no-resume]
 *             [--out box_corr_c]
 *
 ...
 * Checkpoint / resume.
 *
 * Each grid point pt completes an ensemble that fills, for ALL NH box widths,
 * the disjoint slots Cmc[hh*KM+pt]/Cerr[hh*KM+pt], computed purely from
 * (seed, pt) — so checkpointing completed points and skipping them on resume
 * is byte-identical to an uninterrupted run (the seed must be pinned).
 * File <out>/checkpoint.bin: a magic+version+params header, then h_values[NH],
 * T_values[K], then done[K*M], Cmc[NH*K*M], Cerr[NH*K*M].  Written
 * field-group-wise (no struct blob) to avoid padding; written atomically via a
 * .tmp file + rename().
 * ========================================================================= */

#define CKPT_MAGIC   0x42434B32  /* "BCK2" (v2: multi-h) */
#define CKPT_VERSION 2

typedef struct {
    int                N, L, M, nsims, K, iface, NH;
    double             nu;
    unsigned long long seed;          /* the PINNED seed actually used */
    double             h_values[MAX_H];
} CkptParams;
                     ...
                     int *done, double *Cmc, double *Cerr)
{
    ...
    int ok = fread(&magic,   sizeof(int), 1, f) == 1 && magic   == CKPT_MAGIC
          ...
          && fread(&q.NH,    sizeof(int), 1, f) == 1
          && fread(&q.nu,    sizeof(double), 1, f) == 1
          && fread(&q.seed,  sizeof(unsigned long long), 1, f) == 1;
    if (!ok) { fclose(f); return 0; }
    if (q.K < 1 || q.K > MAX_T || q.M < 1 || q.NH < 1 || q.NH > MAX_H) {
        fclose(f); return 0;                 /* implausible -> treat as corrupt */
    }

    double qh[MAX_H], Tv[MAX_T];
    if ((int)fread(qh, sizeof(double), q.NH, f) != q.NH) { fclose(f); return 0; }
    if ((int)fread(Tv, sizeof(double), q.K,  f) != q.K)  { fclose(f); return 0; }

    int match = (q.N == p->N && q.L == p->L && q.M == p->M && q.nsims == p->nsims &&
                 q.K == p->K && q.iface == p->iface && q.NH == p->NH && q.nu == p->nu &&
                 q.seed == p->seed);
    if (match) for (int i = 0; i < q.NH; i++) if (qh[i] != p->h_values[i]) match = 0;
    if (match) for (int i = 0; i < q.K;  i++) if (Tv[i] != T_values[i])    match = 0;
    if (!match) { fclose(f); return -1; }

    if ((int)fread(done, sizeof(int),    KM,   f) != KM ||
        fread(Cmc,  sizeof(double), NHKM, f) != NHKM ||
        fread(Cerr, sizeof(double), NHKM, f) != NHKM) {
        memset(done, 0, (size_t)KM * sizeof(int));   /* short/corrupt -> fresh */
        memset(Cmc,  0, NHKM * sizeof(double));
        memset(Cerr, 0, NHKM * sizeof(double));
        fclose(f);
        return 0;
    }
    fclose(f);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if      (!strcmp(argv[i], "--N")     && i + 1 < argc) N     = atoi(argv[++i]);
            ...
            while (tok && NH < MAX_H) { h_values[NH++] = atof(tok); tok = strtok(NULL, ","); }
        }
    ...
    if (N <= 0 || L <= 0 || nu <= 0 || M <= 0 || nsims <= 0 || K <= 0 || NH <= 0) {
    ...
    for (int hh = 0; hh < NH; hh++) {
        int w = (int)floor(h_values[hh] * N / 2.0 + 0.5);
        if (w < 0) w = 0;
        w_values[hh] = w;
    }
    int c0 = l / 2;
    ...
    printf("  box widths (%d):", NH);
    for (int hh = 0; hh < NH; hh++)
        printf("  h=%g(|B|=%d)", h_values[hh], 2 * w_values[hh] + 1);
    printf("   reference site %d (x=L/2)\n", c0);
    printf("  observable: %s\n",
           iface ? "interface memory f  (N^2 * connected eta_box correlator)"
                 : "spin box correlator");
    printf("  M=%d space pts  K=%d times  nsims=%d per point  seed=%llu\n",
           M, K, nsims, seed);
    printf("  => %d independent ensembles x %d box widths (%ld trajectories total)\n",
           M * K, NH, (long)M * K * nsims);
#ifdef _OPENMP
    printf("  OpenMP: %d threads\n", omp_get_max_threads());
#endif

    /* ---- accumulators: Cmc[hh*KM + ti*M + j] ---- */
    int    KM   = K * M;
    size_t NHKM = (size_t)NH * KM;
    double *Cmc  = calloc(NHKM, sizeof(double));
    double *Cerr = calloc(NHKM, sizeof(double));
    ...
    if (!done || !Cmc || !Cerr) { perror("calloc"); return 1; }
    CkptParams cp;
    cp.N = N; cp.L = L; cp.M = M; cp.nsims = nsims; cp.K = K;
    cp.iface = iface; cp.NH = NH; cp.nu = nu; cp.seed = seed;
    for (int hh = 0; hh < NH; hh++) cp.h_values[hh] = h_values[hh];
    ...
    if (allow_resume) {
        ...
        if (rc == 1)
            ...
            ckpt_write_atomic(out, &cp, T_values, done, Cmc, Cerr); /* persist seed */
        fflush(stdout);
    } else {
        char pth[600]; snprintf(pth, sizeof(pth), "%s/checkpoint.bin", out); unlink(pth);
    }
    time_t last_ckpt = time(NULL);

    /* ---- one independent ensemble per grid point, measured at every h ---- */
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic)
#endif
    for (int pt = 0; pt < KM; pt++) {
        ...
        double acc[MAX_H], acc2[MAX_H], m0[MAX_H];
        for (int hh = 0; hh < NH; hh++) { acc[hh] = 0.0; acc2[hh] = 0.0; }
        ...
        for (int s = 0; s < nsims; s++) {
            ...
            /* reference at t=0 for every box width (interface estimator
             * subtracts p: <eta_box>=p at equilibrium -> connected, low variance). */
            for (int hh = 0; hh < NH; hh++)
                m0[hh] = iface ? box_eta_mean(spins, l, c0, w_values[hh]) - p
                               : box_mean(spins, l, c0, w_values[hh]);

            InterfaceState *gs = istate_alloc(spins, l, N, ann, create, eseed);
            istate_advance_to(gs, T);
            istate_to_spins(gs, spins);
            for (int hh = 0; hh < NH; hh++) {
                double mt = iface ? box_eta_mean(spins, l, centers[j], w_values[hh]) - p
                                  : box_mean(spins, l, centers[j], w_values[hh]);
                double pr = m0[hh] * mt;
                acc[hh]  += pr;
                acc2[hh] += pr * pr;
            }
...
#pragma omp critical
        ...
        {
            for (int hh = 0; hh < NH; hh++) {
                double mean = acc[hh] / nsims;
                double var  = acc2[hh] / nsims - mean * mean;
                Cmc [(size_t)hh * KM + ti * M + j] = sc * mean;
                Cerr[(size_t)hh * KM + ti * M + j] = sc * sqrt(fmax(var, 0.0) / nsims);
            }
            done[pt] = 1;
            done_count++;
            ...
            if (done_count % M == 0)
                printf("  %d/%d grid points done\n", done_count, KM), fflush(stdout);
        }
    }

    /* ---- output: one box_corr.txt + configuration.json per box width ---- */
    char path[700];
    for (int hh = 0; hh < NH; hh++) {
        int boxw = 2 * w_values[hh] + 1;
        char hdir[600];
        snprintf(hdir, sizeof(hdir), "%s/h%g", out, h_values[hh]);
        if (mkdir(hdir, 0755) && errno != EEXIST) { perror("mkdir h"); return 1; }

        snprintf(path, sizeof(path), "%s/box_corr.txt", hdir);
        FILE *f = fopen(path, "w");
        ...
        for (int j = 0; j < M; j++) {
            ...
            for (int ti = 0; ti < K; ti++)
                fprintf(f, "\t%.8f\t%.8f",
                        Cmc [(size_t)hh * KM + ti * M + j],
                        Cerr[(size_t)hh * KM + ti * M + j]);
            fprintf(f, "\n");
        }
        fclose(f);

        snprintf(path, sizeof(path), "%s/configuration.json", hdir);
        f = fopen(path, "w");
        ...
        fprintf(f, "}\n");
        fclose(f);
        printf("Wrote %s/{box_corr.txt,configuration.json}\n", hdir);
    }
